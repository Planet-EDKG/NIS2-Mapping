---
tags: ["IT-Grundschutz"]
status: offen
owner: 
mapping_to: ["000 IT-Grundschutz-Mapping", "A.7.5", "A.7.8", "A.8.14"]
id: A.7.11
category: Technische Maßnahmen
---

# A.7.11: Supporting Utilities
## Versorgungseinrichtungen

**ISO/IEC 27002:2022 - Physische Maßnahmen**

---

## IT-Grundschutz Mapping

### Primär relevant

- **INF.1** - Allgemeines Gebäude
- **INF.2** - Rechenzentrum sowie Serverraum
- **INF.5** - Raum sowie Schrank für technische Infrastruktur
- **INF.12** - Verkabelung

### Zusätzlich relevant

- INF.2.A3 - Einsatz einer unterbrechungsfreien Stromversorgung (UPS)
- INF.2.A5 - Einhaltung der Lufttemperatur und -feuchtigkeit
- INF.2.A10 - Inspektion und Wartung der Infrastruktur
- INF.2.A14 - Einsatz einer Netzersatzanlage (NEA)
- INF.2.A16 - Klimatisierung im Rechenzentrum
- INF.2.A25 - Redundante Auslegung von UPS
- INF.2.A26 - Redundante Auslegung von NEA
- SYS.1.1.A15 - Unterbrechungsfreie und stabile Stromversorgung
- SYS.2.1.A39 - Unterbrechungsfreie und stabile Stromversorgung

---

## Beschreibung

Organisationen müssen zuverlässige Versorgungseinrichtungen sicherstellen:

- Stromversorgung (Hauptversorgung, Backup, Generatoren)
- Klimatisierung und Belüftung
- Feuerlöschsysteme
- Wassersysteme und Drainage
- Redundante Leitungen
- Wartung und Tests
- Kontinuierliche Überwachung

---

## IT-Grundschutz Bezug

Das IT-Grundschutz-Kompendium verlangt umfassende Infrastruktur-Maßnahmen (INF.1, INF.2, INF.5, INF.12) mit UPS, NEA und Klimatisierung.

---

## Related

- [[A.7.5]] - Protecting against environmental threats
- [[A.7.8]] - Equipment siting and protection
- [[A.8.14]] - Redundancy of information processing facilities
- [[000 IT-Grundschutz-Mapping]] - Übersicht
